...
test_drop_removes_last_lines() {
  ...
  printf 'a\nb\nc\nd\ne\n' > "$tmpfile"
  _run_cmd sh -c "cat '$tmpfile' | $ROOT_DIR/spells/.imps/text/drop 2"
  rm -f "$tmpfile"
  _assert_success
  _assert_output_contains "a"
  _assert_output_contains "b"
  _assert_output_contains "c"
  case "$OUTPUT" in
...
test_drop_handles_empty_input() {
  _run_cmd sh -c "printf '' | $ROOT_DIR/spells/.imps/text/drop 1"
  _assert_success
  [ -z "$OUTPUT" ] || { TEST_FAILURE_REASON="output should be empty"; return 1; }
}

_run_test_case "drop removes last N lines" test_drop_removes_last_lines
_run_test_case "drop handles empty input" test_drop_handles_empty_input
