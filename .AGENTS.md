...
> - `.github/BOOTSTRAPPING.md` — **Bootstrapping paradigm and set -eu rules** (essential for AI agents)
...
## Critical: set -eu Placement (Common AI Error)

**See `.github/BOOTSTRAPPING.md` for complete rules.**

### Castable Spells
```sh
spell_name() {
  require-wizardry || return 1  # Before set -eu
  set -eu  # INSIDE function
  env-clear
}
castable "$@"  # After function
```
...
**ONE set -eu per file!** Never duplicate.

### Self-Contained Scripts
```sh
#!/bin/sh
# Setup PATH before set -eu
PATH="/usr/bin:/bin:$PATH"
set -eu
# No wizardry dependencies
```
