...
The `usage-error` imp prints the error to stderr with the spell name prefix and exits
with code 2 (standard usage error). Extract the spell name with `basename "$0"` at
the start of flag parsing.
...
- **Exit code 2 for usage errors**: Use `exit 2` for invalid arguments (standard usage error code)
