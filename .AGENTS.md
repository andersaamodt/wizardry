...
> - `.github/copilot-instructions.md` — Consolidated entry point with templates and critical rules (Copilot)
> - `.github/BOOTSTRAPPING.md` — **NEW**: Bootstrapping and initialization paradigm (essential reading)
...
## Bootstrapping Quick Reference

**See `.github/BOOTSTRAPPING.md` for complete details.**

### The Five Phases

```
Phase 0: POSIX Foundation     → validate with detect-posix
Phase 1: Download & Install   → run ./install
Phase 2: Shell Integration    → install modifies shell rc
Phase 3: Runtime Invocation   → invoke-wizardry sourced at shell startup
Phase 4: Validation           → banish N (optional)
```

### Key Components

| Component | Type | When | Can Use Wizardry? |
|-----------|------|------|-------------------|
| `install` | Bootstrap script | Phase 1-2 | No |
| `invoke-wizardry` | Bootstrap script | Phase 3 | No (but loads it) |
| `require-wizardry` | Imp | Phase 3+ | Yes |
| `banish` | Spell | Phase 4 | Yes |

### Critical Rules

- **Bootstrap scripts** (install, detect-*, spells/install/core/*) cannot use wizardry
- **Regular spells** should use `require-wizardry || return 1` (not exit!)
- **Banish** validates, it doesn't install
- **invoke-wizardry** initializes, it doesn't install
- **require-wizardry** checks, it doesn't initialize
