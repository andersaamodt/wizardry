...
> - `.github/copilot-instructions.md` — Concise project overview (Copilot)
> - `.github/CODEX.md` — Project guidance for OpenAI Codex
...
> - `.github/instructions/logging.instructions.md` — Logging and output standards
...
* **No environment variable overrides**: Spells never expect callers to override behavior by setting environment variables. Configuration is passed via arguments or read from files. All-caps variables are reserved for POSIX standard variables (PATH, HOME, IFS, etc.) and internal wizardry infrastructure (WIZARDRY_*). User-facing data flows through arguments.
...
* **Self-healing failures**: When a spell encounters a missing prerequisite or failed assumption, it must fix the problem automatically or offer to fix it—never quit with an error that tells the user to fix it themselves. This is the core philosophy of wizardry.
* **Never speak imperatively**: Error messages and output must never use imperative language (e.g., "Please install X", "Run Y", "Note: you must..."). Describe what went wrong, then fix it. Never tell the user to do something on their own—taking care of it for the user is the whole point of wizardry.

## Spell Style Guide

Run `lint-magic` to check style compliance:
```sh
lint-magic                              # Check all spells
lint-magic --verbose                    # Show passing files too
lint-magic spells/category/spell-name   # Check specific spell
```
...
### Required Elements (enforced by `lint-magic`)
...
These elements are strongly recommended for all spells (non-imps) and checked by `lint-magic`.
...
#### 6. Flag Parsing with getopts (When Needed)

Most spells only need the help handler above. For spells that accept additional short
flags (like `-v` for verbose), use `getopts` for structured parsing after `set -eu`:
...
while getopts "vh" opt; do
  case $opt in
    v) verbose=1 ;;
    h) show_usage; exit 0 ;;
    *) usage-error "$spell_name" "unknown option: -$OPTARG" ;;
  esac
done
shift $((OPTIND - 1))
```

The `usage-error` imp is a bound imp that prints the error to stderr with the spell
name prefix and returns exit code 2. With `set -e` enabled, this causes the script
to exit immediately. Extract the spell name with `basename "$0"` at the start of
flag parsing.

Guidelines for flag parsing:
- **Keep flags minimal**: More than 2-3 flags suggests refactoring into separate spells
- **Short flags only**: Use `-v` not `--verbose` (long flags need manual parsing)
- **Handle long flags separately**: `--list` style flags are handled via case statement before getopts
- **Exit code 2 for usage errors**: The `usage-error` imp returns code 2 automatically
- **Always shift**: Include `shift $((OPTIND - 1))` to remove parsed flags from `$@`
...
- **Strict mode**: `set -eu` for action imps; **NOT** for conditional imps

**Conditional imps** (those that return exit codes for flow control like `has`, `there`, `is`) do NOT use `set -eu` because they are designed for use in `if` statements and `&&`/`||` chains where non-zero exit codes indicate false rather than error.

Example action imp (uses strict mode):
...
Example conditional imp (no strict mode):
...
Wizardry strictly controls the use of all-caps variable names:

```sh
# CORRECT - lowercase for local/internal variables
spell_name="my-spell"
target_file="$1"
result=$(command)

# CORRECT - standard POSIX variables
PATH="$new_path:$PATH"
HOME="$home_dir"
IFS=':'

# CORRECT - WIZARDRY_* infrastructure variables
WIZARDRY_LOG_LEVEL=2
WIZARDRY_TMPDIR="/tmp/wiz"

# WRONG - all-caps for user-facing data
CONTACTS_DIR="$1"
MARKERS_DIR="/path/to/markers"
SCRIPT_NAME=$(basename "$0")
```

**Rationale**: All-caps variables suggest environment variable overrides, which violates the "arguments over variables" principle. Spells never expect callers to configure behavior by setting environment variables—configuration flows through arguments or files. The only exceptions are:
- POSIX standard variables (PATH, HOME, IFS, CDPATH, etc.)
- Internal WIZARDRY_* infrastructure variables used by the test framework
- Variables in test infrastructure files (test-bootstrap, etc.)
...
```sh
# CORRECT - snake_case for spell functions
show_usage() { ... }
detect_os() { ... }
helper_usable() { ... }
read_value() { ... }
...
The underscore prefix is reserved for imp "true names" to distinguish internal functions from public hyphenated aliases.
...
**Error messages must not be imperative.** Do not tell users what to do—describe what went wrong:
...
When a spell encounters a missing prerequisite, it should fix the problem automatically or offer to fix it—never quit telling the user to fix it themselves.
