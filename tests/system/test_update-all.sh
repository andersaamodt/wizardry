...
test_help() {
run_spell "spells/system/update-all" --help
  assert_success && assert_output_contains "Usage: update-all"
}

test_argument_validation() {
  run_spell "spells/system/update-all" --unknown
  assert_failure && assert_error_contains "Unknown option"

  run_spell "spells/system/update-all" extra
  assert_failure && assert_error_contains "Unexpected argument"
}

test_missing_confirmation_helper() {
  run_cmd env PATH="/bin:/usr/bin" WIZARDRY_UPDATE_ALL_DISTRO=debian "$(pwd)/spells/system/update-all"
  assert_failure
