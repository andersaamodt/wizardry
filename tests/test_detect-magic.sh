...
detect_magic_shows_usage() {
  run_spell "spells/detect-magic" --help
  assert_success || return 1
...
STUB
  ...
  DETECT_MAGIC_READ_MAGIC="$stub" run_spell_in_dir "$tmpdir" "spells/detect-magic"
  assert_success || return 1
...
detect_magic_reports_missing_helper() {
  tmpdir=$(make_tempdir)
  cp "$ROOT_DIR/spells/detect-magic" "$tmpdir/detect-magic"
