...
test_argument_validation() {
  ...
  run_spell "spells/disenchant" a b c
  assert_failure && assert_error_contains "requires one or two arguments"
}

test_missing_file() {
  run_spell "spells/disenchant" "$WIZARDRY_TMPDIR/missing"
...
STUB
  ...
  : >"$tmpfile"
  PATH="$stub_dir:$PATH" run_spell "spells/disenchant" "$tmpfile"
...
STUB
  ...
  PATH="$stub_dir:$PATH" run_spell "spells/disenchant" "$target" user.note
...
STUB
  ...
  PATH="$stub_dir:$PATH" run_spell "spells/disenchant" "$target"
  assert_success
  called=$(cat "$WIZARDRY_TMPDIR/disenchant.call")
...
STUB
  ...
  : >"$target"
  PATH="$stub_dir:/usr/bin:/bin" run_spell "spells/disenchant" "$target"
