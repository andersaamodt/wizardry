...
test_requires_argument() {
  reset_path
  run_spell "spells/read-magic"
  assert_failure && assert_output_contains "requires one or two arguments"
}

test_rejects_extra_argument() {
  reset_path
  run_spell "spells/read-magic" one two three
  assert_failure && assert_output_contains "requires one or two arguments"
}

test_missing_file() {
  reset_path
  run_spell "spells/read-magic" "$WIZARDRY_TMPDIR/does-not-exist"
...
STUB
  chmod +x "$stub_dir/getfattr"
  PATH="$stub_dir:$PATH" run_spell "spells/read-magic" "$target" user.charisma
...
STUB
  chmod +x "$stub_dir/attr"
  PATH="$stub_dir:$PATH" run_spell "spells/read-magic" "$target" user.none
...
test_handles_missing_helpers() {
  ...
  PATH="$stub_dir:$PATH"
  export PATH
  run_spell "spells/read-magic" "$target"
