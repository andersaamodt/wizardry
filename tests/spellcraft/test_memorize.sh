...
test_missing_detect_helper_fails() {
  ...
  PATH=/usr/bin:/bin MEMORIZE_DETECT_RC_FILE="$case_dir/missing" \
    run_spell_in_dir "$case_dir" "spells/spellcraft/memorize" "$spell"
...
test_all_skips_non_installable() {
  ...
  MEMORIZE_DETECT_RC_FILE="$detect_stub" \
    run_spell_in_dir "$case_dir" "spells/spellcraft/memorize" --all "$case_dir"
...
test_alias_records_command() {
  reset_logs
  case_dir=$(make_tempdir)
  store="$case_dir/memorize-command"
  cat >"$store" <<'SCRIPT'
#!/bin/sh
...
SCRIPT
  ...
  MEMORIZE_COMMAND_HELPER="$store" run_spell "spells/spellcraft/memorize" alias zap echo hello

  assert_success
  assert_output_contains "Memorized: zap -> echo hello"
...
test_alias_requires_command() {
  run_spell "spells/spellcraft/memorize" alias onlyname
  assert_failure && assert_error_contains "alias requires a NAME and a COMMAND"
}

test_alias_fails_when_store_missing() {
  reset_logs
  missing="$(make_tempdir)/memorize-command"
  PATH=/usr/bin:/bin MEMORIZE_COMMAND_HELPER="$missing" \
    run_spell "spells/spellcraft/memorize" alias zap echo hi
  assert_failure && assert_error_contains "memorize-command helper is unavailable"
}
...
test_prompt_decline_exits_cleanly() {
  ...
  MEMORIZE_ASK_YN="$ask_stub" run_spell_in_dir "$case_dir" "spells/spellcraft/memorize"
...
test_prompt_accept_scans_directory() {
  ...
  MEMORIZE_ASK_YN="$ask_stub" MEMORIZE_DETECT_RC_FILE="$detect_stub" \
    run_spell_in_dir "$case_dir" "spells/spellcraft/memorize"
...
test_recursive_requires_directory() {
  run_spell "spells/spellcraft/memorize" --recursive
...
test_install_failures_are_reported() {
  ...
  MEMORIZE_DETECT_RC_FILE="$detect_stub" run_spell "spells/spellcraft/memorize" "$failing"
...
test_detect_stub_without_rc_fails() {
  ...
  MEMORIZE_DETECT_RC_FILE="$detect_stub" run_spell "spells/spellcraft/memorize" "$spell"
...
run_test_case "memorize fails when memorize-command is missing" test_alias_fails_when_store_missing
