...
test_argument_validation() {
  ...
  run_spell "spells/enchantment-to-yaml" one two
  assert_failure && assert_error_contains "incorrect number of arguments"
}

test_missing_file() {
  run_spell "spells/enchantment-to-yaml" "$WIZARDRY_TMPDIR/missing"
...
STUB
  ...
  printf 'body\n' >"$target"
  PATH="$stub_dir:$PATH" run_spell "spells/enchantment-to-yaml" "$target"
...
STUB
  ...
  PATH="$stub_dir:/usr/bin:/bin" run_spell "spells/enchantment-to-yaml" "$target"
