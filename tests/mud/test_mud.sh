...
EOF
  ...
  run_cmd env PATH="$stub_dir:$PATH" REQUIRE_COMMAND="$stub_dir/require-command" "$(pwd)/spells/mud/mud"
...
EOF
  ...
  run_cmd env LOG_PATH="$log" COUNT_FILE="$counter" PATH="$stub_dir:$PATH" REQUIRE_COMMAND="$stub_dir/require-command" "$(pwd)/spells/mud/mud"
  assert_success
