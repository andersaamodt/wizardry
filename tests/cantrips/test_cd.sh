...
test_cd_install_command_installs_without_prompting() {
  ...
  run_cmd env WIZARDRY_CD_CANTRIP="$ROOT_DIR/spells/cantrips/cd" WIZARDRY_RC_FILE="$tmp/rc" "$ROOT_DIR/spells/cantrips/cd" install
  assert_success && assert_path_exists "$tmp/rc" && assert_output_contains "installed wizardry hooks"
...
SH
  ...
  run_cmd env PATH="$tmp:$PATH" WIZARDRY_RC_FILE="$tmp/rc" "$ROOT_DIR/spells/cantrips/cd" "$nonexistent"
  assert_failure
}
...
test_cd_uses_shell_specific_rc_file() {
  ...
  # Test with zsh
  run_cmd env WIZARDRY_CD_CANTRIP="$ROOT_DIR/spells/cantrips/cd" HOME="$tmp" SHELL="/bin/zsh" "$ROOT_DIR/spells/cantrips/cd" install
  assert_success && assert_path_exists "$tmp/.zshrc"
  
  # Test with bash
  tmp2=$(make_tempdir)
  run_cmd env WIZARDRY_CD_CANTRIP="$ROOT_DIR/spells/cantrips/cd" HOME="$tmp2" SHELL="/bin/bash" "$ROOT_DIR/spells/cantrips/cd" install
...
EOF
  ...
  run_cmd env WIZARDRY_CD_CANTRIP="$ROOT_DIR/spells/cantrips/cd" HOME="$tmp" "$ROOT_DIR/spells/cantrips/cd" install
