...
test_help() {
  run_spell "spells/arcane/look" --help
  assert_success && assert_output_contains "Usage: look"
}

test_missing_read_magic() {
  stub=$(make_stub_dir)
  PATH="$ROOT_DIR/spells/.imps:$stub:/bin:/usr/bin" run_spell "spells/arcane/look" "$WIZARDRY_TMPDIR"
...
EOF
  ...
  PATH="$stub:/bin:/usr/bin" run_spell "spells/arcane/look" "$WIZARDRY_TMPDIR"
...
EOF
  ...
  LOOK_RC_FILE="$rc_file" PATH="$ROOT_DIR/spells/.imps:$stub:/bin:/usr/bin" run_spell "spells/arcane/look" "$WIZARDRY_TMPDIR"
...
EOF
  ...
  ASK_LOG="$prompt_log" LOOK_RC_FILE="$rc_file" PATH="$ROOT_DIR/spells/.imps:$stub:/bin:/usr/bin" run_spell "spells/arcane/look" "$WIZARDRY_TMPDIR"
...
EOF
  before=$(cat "$rc_file")
  LOOK_RC_FILE="$rc_file" PATH="$stub:/bin:/usr/bin" run_spell "spells/arcane/look" "$WIZARDRY_TMPDIR"
