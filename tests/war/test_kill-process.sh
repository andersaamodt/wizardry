...
test_help() {
  run_spell "spells/war/kill-process" --help
...
STUB
  ...
  PATH="$tmpdir:/bin:/usr/bin" run_spell "spells/war/kill-process"
...
STUB
  ...
  PATH="$tmpdir:/bin:/usr/bin" KILL_LOG="$kill_log" KILL_CMD="$tmpdir/kill" run_spell "spells/war/kill-process"
  assert_success || return 1
...
STUB
  ...
  PATH="$tmpdir:/bin:/usr/bin" KILL_CMD="$tmpdir/not-here" run_spell "spells/war/kill-process"
  assert_failure || return 1
...
STUB
  ...
  PATH="$tmpdir:/bin:/usr/bin" KILL_CMD="$tmpdir/kill" run_spell "spells/war/kill-process"
  assert_failure || return 1
