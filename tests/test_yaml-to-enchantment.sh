...
test_argument_validation() {
  ...
  run_spell "spells/yaml-to-enchantment" one two
  assert_failure && assert_error_contains "incorrect number of arguments"
}

test_missing_file() {
  run_spell "spells/yaml-to-enchantment" "$WIZARDRY_TMPDIR/missing"
...
test_requires_header() {
  ...
  printf 'content\n' >"$tmpfile"
  run_spell "spells/yaml-to-enchantment" "$tmpfile"
...
FILE
  ...
  PATH="$stub_dir:$PATH" run_spell "spells/yaml-to-enchantment" "$tmpfile"
  assert_success
...
FILE
  ...
  PATH="/usr/bin:/bin" run_spell "spells/yaml-to-enchantment" "$tmpfile"
