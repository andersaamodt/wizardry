...
- Parse MUST use 'command' builtin (not exec) to bypass functions/aliases when falling back to system commands; this prevents infinite recursion (gloss → parse → exec gloss → parse ...) causing segfault/exit 139, and 'command' is POSIX-compliant unlike 'type' builtin.
- When exit 139 (segfault) occurs AFTER successful command output, it indicates shell cleanup crash not code failure; use subshell pattern `(command) && passed=true || passed=false; [ "$passed" = "true" ]` to capture exit code before cleanup.
- Nix-shell segfaults during cleanup/teardown when sourcing large amounts of shell code (~21K lines of function definitions from generate-glosses); this is an environment limitation not a code bug.
- Binary search debugging (testing banish at levels 8→4→1→0) combined with `set -x` tracing effectively isolates whether failures occur during execution vs during shell cleanup/teardown.
- Use `set -x` trace output to distinguish code execution failures from shell cleanup failures; if last traced command completes successfully but process exits 139, the segfault is in cleanup not code.
