...
- Pandoc wraps inline HTML elements (like `<strong>` and `<button>`) in `<p>` tags when they're direct children of a block element in Markdown files; wrap them in a proper block-level container (div) to prevent this and maintain intended layout (commit 67685c7 used display:contents workaround instead).
