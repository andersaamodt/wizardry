...
- Sourced scripts (when cd-listen enabled or similar) must use explicit `set +e; set +u` at ALL return points instead of `eval "$(set +o)"` - the eval restore re-enables strict modes (set -e, set -u) that break zsh's line editor (zle), causing arrow keys to show escape sequences (^[[A); applies to listen, env-clear, and any task script sourced into interactive shell.
