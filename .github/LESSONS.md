...
- Parse must strip carriage returns from `parse-enabled` config values so CRLF `.mud` files cannot bypass a disabled parser.
...
- Sourced scripts with save/restore MUST restore readline mode at ALL return points, not just the final one - missing readline restoration at early returns (error paths) will break readline in the caller's shell; every eval "$saved_opts" must be followed by set -o emacs/vi restoration.
...
- **ANTIPATTERN:** NEVER wrap builtin commands with eval to hide from checkbashisms - `eval 'builtin cd "$@"'` breaks argument expansion with relative paths; use `builtin cd "$@"` directly with checkbashisms exemption comments instead (see EXEMPTIONS.md).
- Task scripts sourced into interactive shells must use `set +e; set +u` at ALL return points instead of `eval "$(set +o)"` - eval restore re-enables strict modes breaking shell editing; applies to listen, env-clear, etc.
...
- Blog feed and robots generators must revalidate imported `domain` and `https` values before writing static artifacts, not rely on nginx config validation.
- Tor hidden-service updaters must parse `HiddenServiceDir` records and compare final path segments literally; dotted site names are regex patterns if interpolated into grep.
...
- Test harnesses should prefer sourced-command status over exported success sentinels; avoid adding shell variables just to prove a helper returned successfully.
