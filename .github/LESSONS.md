...
- Pandoc wraps inline HTML elements in paragraph tags when they appear on separate lines in Markdown files; place all related inline elements on a single line to prevent this and maintain intended layout.
- To prevent Pandoc from injecting <p> tags around HTML elements in Markdown files, consolidate all related inline elements onto a single line with no line breaks between them; this signals to Pandoc that the HTML is intentional inline markup rather than block-level content requiring paragraph wrapping.
