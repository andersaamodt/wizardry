...
**Fix**: Created `_iw_debug_log()` helper function with safe timestamp handling:

```sh
_iw_debug_log() {
  if [ -n "$_iw_debug_file" ]; then
    _iw_ts=$(date '+%Y-%m-%d %H:%M:%S' 2>/dev/null || printf 'NO-DATE')
    printf '%s\n' "[${_iw_ts}] invoke-wizardry: $*" >> "$_iw_debug_file" 2>/dev/null || :
  fi
}
```
...
- `spells/.imps/sys/invoke-wizardry` - Removed self-execute pattern, improved debug logging
