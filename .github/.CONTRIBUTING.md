...
2. **Follow the spell template** - See `.github/.AGENTS.md` for the complete spell style guide
3. **Write tests** - Every spell needs a test in `.tests/` that mirrors its path
4. **Run the vetting** - Use `./spells/spellcraft/lint-magic` to check style compliance
...
- **Error messages**: Descriptive, not imperative (see `.github/.AGENTS.md`)
...
See the [spell style guide](/.github/.AGENTS.md) for complete requirements.
