...
test_get_returns_disabled_by_default() {
  tmp=$(make_tempdir)
  run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/.arcana/mud/mud-config" get combat
  assert_success || return 1
...
test_set_enables_feature() {
  tmp=$(make_tempdir)
  run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/.arcana/mud/mud-config" set combat 1
  assert_success || return 1
...
test_toggle_flips_state() {
  ...
  run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/.arcana/mud/mud-config" toggle combat
  assert_success || return 1
  assert_output_contains "1" || return 1
...
test_invalid_value_rejected() {
  tmp=$(make_tempdir)
  run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/.arcana/mud/mud-config" set combat invalid
