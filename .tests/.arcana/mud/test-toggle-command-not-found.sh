...
test_toggle_enables_feature() {
  tmp=$(_make_tempdir)
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/.arcana/mud/toggle-command-not-found"
  _assert_success || return 1
  _assert_output_contains "enabled" || return 1
  
  # Verify state
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/.arcana/mud/mud-config" get command-not-found
  _assert_success || return 1
