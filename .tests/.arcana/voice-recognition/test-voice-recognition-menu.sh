...
test_menu_shows_macos_toggles() {
  ...
  case "$args" in
    *"Voice Recognition:"*"[ ] Faster-Whisper%WIZARDRY_LOG_LEVEL=1 $ROOT_DIR/spells/.arcana/voice-recognition/install-faster-whisper"*"[ ] MLX-Whisper%WIZARDRY_LOG_LEVEL=1 $ROOT_DIR/spells/.arcana/voice-recognition/install-mlx-whisper"*'Exit%kill -TERM $PPID'*) : ;;
    *)
...
test_menu_shows_parakeet_toggle_on_linux_nvidia() {
  ...
  args=$(cat "$tmp/menu.log")
  case "$args" in
    *"Voice Recognition:"*"[ ] Faster-Whisper%WIZARDRY_LOG_LEVEL=1 $ROOT_DIR/spells/.arcana/voice-recognition/install-faster-whisper"*"[ ] Parakeet%WIZARDRY_LOG_LEVEL=1 $ROOT_DIR/spells/.arcana/voice-recognition/install-parakeet"*'Exit%kill -TERM $PPID'*) : ;;
    *)
