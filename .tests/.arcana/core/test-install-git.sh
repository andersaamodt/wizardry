...
install_git_reports_failure_when_package_manager_fails() {
  ...
  assert_error_contains "unable to install git automatically" || return 1
}
...
run_test_case "install-git reports failure when package manager fails" install_git_reports_failure_when_package_manager_fails


shows_help() {
  run_spell spells/.arcana/core/install-git --help
  true
}
