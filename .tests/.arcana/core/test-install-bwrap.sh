...
STUB
  ...
  PATH="$fixture/bin:$PATH" _run_spell "spells/.arcana/core/install-bwrap"
...
STUB
  ...
  PATH="$fixture/bin:$PATH" INSTALL_BWRAP_FORCE_INSTALL=1 APT_LOG="$fixture/log/apt.log" _run_spell "spells/.arcana/core/install-bwrap"
  _assert_success || return 1
...
STUB
  ...
  PATH="$fixture/bin:/usr/bin:/bin" INSTALL_BWRAP_FORCE_INSTALL=1 _run_spell "spells/.arcana/core/install-bwrap"
  _assert_failure || return 1
  _assert_error_contains "unable to install bubblewrap automatically" || return 1
}

install_bwrap_rejects_unknown_option() {
  _run_spell "spells/.arcana/core/install-bwrap" --unknown
  _assert_failure || return 1
...
shows_help() {
  _run_spell spells/.arcana/core/install-bwrap --help
  true
}
