...
test_uninstall_git_is_executable() {
  [ -x "$ROOT_DIR/spells/.arcana/core/uninstall-git" ]
}

test_uninstall_git_uses_manage_system_command() {
  content=$(cat "$ROOT_DIR/spells/.arcana/core/uninstall-git")
  case "$content" in
...
test_uninstall_git_reports_failure_when_package_manager_fails() {
  ...
  write_command_stub "$fixture/bin" git
  ...
  PATH="$fixture/bin" HOME="$fixture/home" APT_LOG="$fixture/log/apt.log" APT_EXIT=1 run_cmd \
    ...
    "$ROOT_DIR/spells/.arcana/core/uninstall-git"

  assert_failure || return 1
  assert_error_contains "unable to uninstall git automatically" || return 1
}
...
run_test_case "uninstall-git reports failure when package manager fails" test_uninstall_git_reports_failure_when_package_manager_fails


shows_help() {
  run_spell spells/.arcana/core/uninstall-git --help
  true
}
