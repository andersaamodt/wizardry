...
test_bitcoin_menu_prompts_install_when_missing() {
  ...
  _run_cmd env PATH="$tmp:$PATH" MENU_LOG="$tmp/log" "$ROOT_DIR/spells/install/bitcoin-menu"
  _assert_success
...
SH
  ...
  _run_cmd env PATH="$tmp:$PATH" MENU_LOG="$tmp/log" "$ROOT_DIR/spells/install/bitcoin-menu"
  _assert_success
  _assert_file_contains "$tmp/log" "Install Bitcoin Service%install-service-template $ROOT_DIR/spells/install/bitcoin/bitcoin.service \"BITCOIND=/usr/bin/bitcoind\""
  _assert_file_contains "$tmp/log" "Uninstall Bitcoin%uninstall-bitcoin"
}
...
test_shows_help() {
  _run_cmd "$ROOT_DIR/spells/install/bitcoin-menu" --help
  _assert_success
