...
test_help_shows_usage() {
  _run_spell "spells/install/mud/toggle-all-mud" --help
...
test_enable_flag_enables_all() {
  tmp=$(_make_tempdir)
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/install/mud/toggle-all-mud" --enable
  _assert_success || return 1
...
test_disable_flag_disables_all() {
  ...
  # Then disable all
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/install/mud/toggle-all-mud" --disable
  _assert_success || return 1
...
test_auto_toggle_enables_when_any_disabled() {
  ...
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/install/mud/toggle-all-mud"
  _assert_success || return 1
  _assert_output_contains "All MUD features enabled" || return 1
}
