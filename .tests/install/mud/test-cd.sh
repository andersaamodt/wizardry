...
SH
  ...
  _run_cmd env PATH="$tmp:$PATH" WIZARDRY_RC_FILE="$tmp/rc" "$ROOT_DIR/spells/install/mud/cd" "$tmp"
...
SH
  ...
  _run_cmd env PATH="$tmp:$PATH" WIZARDRY_RC_FILE="$tmp/rc" "$ROOT_DIR/spells/install/mud/cd" "$target"
...
SH
  ...
  _run_cmd env PATH="$tmp:$PATH" WIZARDRY_RC_FILE="$tmp/rc" "$ROOT_DIR/spells/install/mud/cd" "$nonexistent"
  _assert_failure
}
...
test_cd_uses_shell_specific_rc_file() {
  ...
  _run_cmd sh -c "PATH=/bin:/usr/bin HOME='$tmp' SHELL='/bin/zsh' '$ROOT_DIR/spells/install/mud/cd' install"
  _assert_success && _assert_path_exists "$tmp/.zshrc"
  
  # Test with bash - use a restricted PATH without detect-rc-file
  tmp2=$(_make_tempdir)
  _run_cmd sh -c "PATH=/bin:/usr/bin HOME='$tmp2' SHELL='/bin/bash' '$ROOT_DIR/spells/install/mud/cd' install"
...
EOF
  ...
  _run_cmd env HOME="$tmp" "$ROOT_DIR/spells/install/mud/cd" install
...
test_cd_uninstall_removes_hook() {
  ...
  # Uninstall the hook
  _run_cmd env WIZARDRY_RC_FILE="$tmp/rc" "$ROOT_DIR/spells/install/mud/cd" uninstall
  _assert_success || return 1
...
test_cd_help_shows_usage() {
  _run_cmd "$ROOT_DIR/spells/install/mud/cd" --help
  _assert_success || return 1
...
STUB
  ...
  _run_cmd env PATH="$tmp:$PATH" WIZARDRY_RC_FORMAT=nix WIZARDRY_RC_FILE="$nix_config" WIZARDRY_SKIP_NIX_REBUILD=1 HOME="$tmp" "$ROOT_DIR/spells/install/mud/cd" install
  _assert_success || return 1
...
EOF
  ...
  _run_cmd env PATH="$tmp:$PATH" WIZARDRY_SKIP_NIX_REBUILD=1 HOME="$tmp" "$ROOT_DIR/spells/install/mud/cd" install
  _assert_success || return 1
...
EOF
  ...
  _run_cmd env PATH="$tmp:$PATH" WIZARDRY_SKIP_NIX_REBUILD=1 HOME="$tmp" "$ROOT_DIR/spells/install/mud/cd" uninstall
  _assert_success || return 1
  _assert_output_contains "uninstalled wizardry hooks" || return 1
