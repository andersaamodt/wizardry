...
test_help_shows_usage() {
  _run_spell "spells/install/mud/mud-config" --help
...
test_get_returns_disabled_by_default() {
  tmp=$(_make_tempdir)
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/install/mud/mud-config" get combat
  _assert_success || return 1
...
test_set_enables_feature() {
  tmp=$(_make_tempdir)
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/install/mud/mud-config" set combat 1
  _assert_success || return 1
...
test_toggle_flips_state() {
  ...
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/install/mud/mud-config" toggle combat
  _assert_success || return 1
  _assert_output_contains "1" || return 1
...
test_list_shows_all_features() {
  tmp=$(_make_tempdir)
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/install/mud/mud-config" list
  _assert_success || return 1
...
test_invalid_value_rejected() {
  tmp=$(_make_tempdir)
  _run_cmd env MUD_DIR="$tmp" "$ROOT_DIR/spells/install/mud/mud-config" set combat invalid
