...
_run_test_case "install/simplex-chat/simplex-chat-status is executable" spell_is_executable

shows_usage_with_help_flag() {
  _run_cmd "$ROOT_DIR/spells/install/simplex-chat/simplex-chat-status" --help
  _assert_success || return 1
  # Usage text is written to stderr to match other arcanum scripts.
  _assert_error_contains "Usage: simplex-chat-status" || return 1
}

_run_test_case "simplex-chat-status shows usage with --help" shows_usage_with_help_flag
...
_run_test_case "simplex-chat-status reports not installed when binary missing" reports_not_installed_without_binary
...
_run_test_case "simplex-chat-status reports installed when version works and directories exist" reports_installed_with_directories
...
_run_test_case "simplex-chat-status warns when data directory is missing" warns_when_setup_missing
...
_run_test_case "simplex-chat-status reports version errors" reports_error_when_version_fails
