...
test_nix_shell_init_help() {
  run_spell "spells/.imps/os/nix-shell-init" --help
  assert_success
...
test_nix_shell_init_add_creates_block() {
  ...
  # Add shell init code
  result=$(printf 'source "/path/to/spell"' | HOME="$tmpdir" "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --shell bash --name testspell --file "$nix_file" 2>&1) || {
...
test_nix_shell_init_add_is_idempotent() {
  ...
  printf 'source "/path/to/spell"' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --shell bash --name testspell --file "$nix_file"
  printf 'source "/path/to/spell"' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --shell bash --name testspell --file "$nix_file"
...
test_nix_shell_init_status_returns_correct_result() {
  ...
  if "$ROOT_DIR/spells/.imps/os/nix-shell-init" status --shell bash --name testspell --file "$nix_file" 2>/dev/null; then
...
test_nix_shell_init_remove_clears_block() {
  ...
  # Remove it
  "$ROOT_DIR/spells/.imps/os/nix-shell-init" remove --shell bash --name testspell --file "$nix_file"
...
test_nix_shell_init_zsh_uses_correct_option() {
  ...
  printf 'source "/path/to/spell"' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --shell zsh --name testspell --file "$nix_file"
...
test_nix_shell_init_multiline_code() {
  ...
  printf 'if [ -f "/path/to/spell" ]; then\n  source "/path/to/spell"\nfi' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --shell bash --name testspell --file "$nix_file"
...
test_nix_shell_init_requires_action() {
  if "$ROOT_DIR/spells/.imps/os/nix-shell-init" --name test --file /tmp/test.nix 2>/dev/null; then
...
test_nix_shell_init_requires_name() {
  if printf 'test' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --file /tmp/test.nix 2>/dev/null; then
...
test_nix_shell_init_requires_file() {
  if printf 'test' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --name test 2>/dev/null; then
...
test_nix_shell_init_escapes_special_chars() {
  ...
  printf 'echo "${HOME}/test"' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --shell bash --name escapespell --file "$nix_file"
...
test_nix_shell_init_only_marks_content_lines() {
  ...
  printf 'export FOO=bar\nexport BAZ=qux' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --shell bash --name marktest --file "$nix_file"
...
EOF
  ...
  printf 'export WIZARDRY_VAR=magic' | "$ROOT_DIR/spells/.imps/os/nix-shell-init" add --shell bash --name inserttest --file "$nix_file"
