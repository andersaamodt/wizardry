...
test_xattr_write_value_exists() {
  run_cmd sh -c 'command -v set-attribute'
...
STUB
  ...
  run_cmd sh -c 'export PATH="'"$tmpdir"'/bin:$PATH" && set-attribute user.name Alice "'"$testfile"'"'
  assert_success || return 1
...
STUB
  ...
  run_cmd sh -c 'export PATH="'"$tmpdir"'/bin:$PATH" && set-attribute user.email bob@example.com "'"$testfile"'"'
  assert_success || return 1
...
STUB
  ...
  run_cmd sh -c 'export PATH="'"$tmpdir"'/bin:$PATH" && set-attribute user.key value "'"$testfile"'"'
  assert_failure || return 1
  
  rm -rf "$tmpdir"
}

run_test_case "set-attribute exists" test_xattr_write_value_exists
run_test_case "set-attribute with mock attr" test_xattr_write_value_with_mock_attr
run_test_case "set-attribute fallback to xattr" test_xattr_write_value_fallback_to_xattr
run_test_case "set-attribute returns error when no helpers" test_xattr_write_value_returns_error_when_no_helpers
