...
test_xattr_read_value_exists() {
  run_cmd sh -c 'command -v attr-get'
...
STUB
  ...
  run_cmd sh -c 'export PATH="'"$tmpdir"'/bin:$PATH" && attr-get user.name "'"$testfile"'"'
  assert_success || return 1
...
STUB
  ...
  run_cmd sh -c 'export PATH="'"$tmpdir"'/bin:$PATH" && attr-get nonexistent.key "'"$testfile"'"'
  assert_failure || return 1
  
  rm -rf "$tmpdir"
}

run_test_case "attr-get exists" test_xattr_read_value_exists
run_test_case "attr-get with mock xattr" test_xattr_read_value_with_mock_xattr
run_test_case "attr-get fallback to attr" test_xattr_read_value_fallback_to_attr
run_test_case "attr-get returns error for missing key" test_xattr_read_value_returns_error_for_missing_key
