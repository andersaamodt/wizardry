...
test_format_minutes_and_seconds() {
  ...
  assert_equals "2m 5s" "$result"
}

test_format_hours_minutes_seconds() {
  result=$(format-duration 3665)
  assert_equals "1h 1m 5s" "$result"
}

test_format_days_hours_minutes_seconds() {
  result=$(format-duration 90061)
  assert_equals "1d 1h 1m 1s" "$result"
}

test_format_exact_minute() {
  result=$(format-duration 120)
  assert_equals "2m" "$result"
}

test_format_exact_hour() {
  result=$(format-duration 7200)
  assert_equals "2h" "$result"
}

test_format_zero_seconds() {
  result=$(format-duration 0)
  assert_equals "0s" "$result"
}

test_format_large_duration() {
  result=$(format-duration 186543)
  assert_equals "2d 3h 49m 3s" "$result"
}
