...
test_detects_installable_spell() {
  ...
  run_spell spells/.imps/is-installable "$spell"
  rm -f "$spell"
  assert_success
}
...
test_fails_for_missing_file() {
  run_spell spells/.imps/is-installable "/nonexistent/path/to/spell"
  assert_failure
}

test_fails_for_empty_argument() {
  run_spell spells/.imps/is-installable ""
  assert_failure
}
...
SPELL
  chmod +x "$tmpfile"
  run_spell spells/.imps/is-installable "$tmpfile"
  rm -f "$tmpfile"
  assert_success
}
