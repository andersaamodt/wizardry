...
test_disable_palette_clears_colors() {
  # Source disable-palette to define function, then call it and check that variables are cleared
  run_cmd sh -c "RESET=1; . '$ROOT_DIR/spells/.imps/out/disable-palette'; disable_palette; printf '%s' \"\$RESET\""
  assert_success
...
test_disable_palette_clears_theme_colors() {
  run_cmd sh -c "THEME_HIGHLIGHT=1; . '$ROOT_DIR/spells/.imps/out/disable-palette'; disable_palette; printf '%s' \"\$THEME_HIGHLIGHT\""
  assert_success
...
test_disable_palette_clears_mud_colors() {
  run_cmd sh -c "MUD_LOCATION=1; . '$ROOT_DIR/spells/.imps/out/disable-palette'; disable_palette; printf '%s' \"\$MUD_LOCATION\""
  assert_success
...
test_disable_palette_sets_flag() {
  ...
  run_cmd sh -c "WIZARDRY_COLORS_AVAILABLE=1; . '$ROOT_DIR/spells/.imps/out/disable-palette'; disable_palette; printf '%s' \"\$WIZARDRY_COLORS_AVAILABLE\""
  assert_success
...
test_disable_palette_multiple_colors_empty() {
  ...
  run_cmd sh -c "RED=1 GREEN=1 BLUE=1 CYAN=1; . '$ROOT_DIR/spells/.imps/out/disable-palette'; disable_palette; printf '%s|%s|%s|%s' \"\$RED\" \"\$GREEN\" \"\$BLUE\" \"\$CYAN\""
  assert_success
