...
test_declare_globals_allows_set_u() {
  ...
  run_cmd sh -c "set -u; . '$ROOT_DIR/spells/.imps/declare-globals'; declare_globals; : \"\$WIZARDRY_DIR\""
  assert_success
}
...
test_declare_globals_sets_empty_defaults() {
  ...
  run_cmd sh -c "unset WIZARDRY_DIR; . '$ROOT_DIR/spells/.imps/declare-globals'; declare_globals; printf '%s' \"\$WIZARDRY_DIR\""
  assert_success
...
test_declare_globals_preserves_existing_values() {
  ...
  run_cmd sh -c "WIZARDRY_DIR=/test/path; . '$ROOT_DIR/spells/.imps/declare-globals'; declare_globals; printf '%s' \"\$WIZARDRY_DIR\""
  assert_success
