...
# - activate-service prompts then enables unit
# - activate-service fails when name missing
...
enable_service_prompts_and_invokes() {
  ...
  PATH="$stub_dir:$PATH" run_spell "spells/system/activate-service"
...
run_test_case "activate-service prompts then enables unit" enable_service_prompts_and_invokes
run_test_case "activate-service fails when name missing" enable_service_requires_name

spell_is_executable() {
  [ -x "$ROOT_DIR/spells/system/activate-service" ]
}

run_test_case "cantrips/activate-service is executable" spell_is_executable
shows_help() {
  run_spell spells/system/activate-service --help
  # Note: spell may not have --help implemented yet
  true
}

run_test_case "activate-service shows help" shows_help
