...
test_config_set_rejects_invalid_key_chars() {
  skip-if-compiled || return $?
  run_cmd "$ROOT_DIR/spells/.imps/fs/config-set" "/tmp/x" "key with spaces" "value"
  assert_failure || return 1
  assert_error_contains "invalid key format" || return 1
}

test_config_set_rejects_key_with_equals() {
  skip-if-compiled || return $?
  run_cmd "$ROOT_DIR/spells/.imps/fs/config-set" "/tmp/x" "key=bad" "value"
  assert_failure || return 1
  assert_error_contains "key cannot contain =" || return 1
}

test_config_set_rejects_newline_in_value() {
  skip-if-compiled || return $?
  # Run directly, not through run_cmd sandbox
...
test_config_get_retrieves_value() {
  ...
  '
  assert_success || return 1
  assert_output_contains "value1" || return 1
}
...
test_config_get_handles_equals_in_value() {
  ...
  '
  assert_success || return 1
  assert_output_contains "a=b=c" || return 1
}

test_config_get_returns_empty_for_missing_file() {
  skip-if-compiled || return $?
  run_cmd "$ROOT_DIR/spells/.imps/fs/config-get" "/tmp/nonexistent-$$.config" "key1"
  assert_success || return 1
  [ -z "$OUTPUT" ] || return 1
}
...
test_config_get_rejects_invalid_key() {
  skip-if-compiled || return $?
  run_cmd "$ROOT_DIR/spells/.imps/fs/config-get" "/tmp/x" "bad key"
  assert_failure || return 1
  assert_error_contains "invalid key format" || return 1
}
...
test_config_has_returns_false_for_missing_file() {
  skip-if-compiled || return $?
  run_cmd "$ROOT_DIR/spells/.imps/fs/config-has" "/tmp/nonexistent-$$.config" "key1"
  assert_failure || return 1
}
...
test_config_del_handles_missing_file() {
  skip-if-compiled || return $?
  run_cmd "$ROOT_DIR/spells/.imps/fs/config-del" "/tmp/nonexistent-$$.config" "key1"
  assert_success || return 1
}
...
run_test_case "config-set updates existing key" test_config_set_updates_existing_key
run_test_case "config-set handles equals in value" test_config_set_handles_equals_in_value
run_test_case "config-set rejects invalid key chars" test_config_set_rejects_invalid_key_chars
run_test_case "config-set rejects key with equals" test_config_set_rejects_key_with_equals
run_test_case "config-set rejects newline in value" test_config_set_rejects_newline_in_value

run_test_case "config-get retrieves value" test_config_get_retrieves_value
run_test_case "config-get handles equals in value" test_config_get_handles_equals_in_value
run_test_case "config-get returns empty for missing file" test_config_get_returns_empty_for_missing_file
run_test_case "config-get rejects invalid key" test_config_get_rejects_invalid_key

run_test_case "config-has returns true when exists" test_config_has_returns_true_when_exists
run_test_case "config-has returns false when missing" test_config_has_returns_false_when_missing
run_test_case "config-has returns false for missing file" test_config_has_returns_false_for_missing_file

run_test_case "config-del removes key" test_config_del_removes_key
run_test_case "config-del handles missing file" test_config_del_handles_missing_file

run_test_case "config roundtrip with special chars" test_config_roundtrip_special_chars
run_test_case "config handles many keys (20)" test_config_many_keys
