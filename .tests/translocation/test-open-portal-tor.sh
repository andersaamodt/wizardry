#!/bin/sh
# Test coverage for open-portal-tor spell:
...
test_help() {
  run_spell "spells/translocation/open-portal-tor" --help
...
test_requires_sshfs() {
  ...
  PATH="$WIZARDRY_IMPS_PATH:$ROOT_DIR/spells/cantrips:$stubdir:/bin:/usr/bin" run_spell "spells/translocation/open-portal-tor"
...
EOF
  ...
  MUD_PLAYER="" PATH="$stubdir:$WIZARDRY_IMPS_PATH:$ROOT_DIR/spells/cantrips:/bin:/usr/bin" run_spell "spells/translocation/open-portal-tor"
  assert_failure || return 1
  assert_error_contains "MUD_PLAYER" || return 1
}

run_test_case "open-portal-tor shows usage text" test_help
run_test_case "open-portal-tor requires sshfs" test_requires_sshfs
run_test_case "open-portal-tor requires MUD_PLAYER" test_requires_mud_player
