...
test_help() {
  _run_spell "spells/cantrips/restart-ssh" --help
  _assert_success || return 1
  _assert_output_contains "Usage: restart-ssh" || return 1
}

test_help_h_flag() {
  _run_spell "spells/cantrips/restart-ssh" -h
  _assert_success || return 1
  _assert_output_contains "Usage: restart-ssh" || return 1
}
...
_run_test_case "restart-ssh shows usage text" test_help
...
restart_ssh_help_via_sourcing() {
  _run_sourced_spell restart-ssh --help
  _assert_success || return 1
...
_run_test_case "restart-ssh works via source-then-invoke" restart_ssh_help_via_sourcing
_finish_tests
