...
# Skip ESC test if /dev/tty is not functional
if [ "${WIZARDRY_TEST_IN_POCKET-0}" = "1" ]; then
  test_skip "menu exits with 130 on ESC" "requires functional /dev/tty"
elif ! stty -g </dev/tty >/dev/null 2>&1; then
  test_skip "menu exits with 130 on ESC" "requires functional /dev/tty"
else
  # Test that ESC exits menu with code 130
  menu_esc_exits_with_130() {
    tmpdir=$(make_tempdir)
    
    stub_dir="$tmpdir/stubs"
    mkdir -p "$stub_dir"
    
    # Create custom await-keypress that returns ESC
    cat > "$stub_dir/await-keypress" <<'STUB_EOF'
#!/bin/sh
printf 'escape\n'
STUB_EOF
    ...
    # Run menu and verify exit code 130
    PATH="$stub_dir:$ROOT_DIR/spells/cantrips:$WIZARDRY_IMPS_PATH:/bin:/usr/bin" run_cmd \
      "$ROOT_DIR/spells/cantrips/menu" "Test:" "Item%printf selected"
    
    # Menu should exit with 130 when ESC is pressed
    if [ "$STATUS" -eq 130 ]; then
      return 0
    else
      TEST_FAILURE_REASON="expected exit code 130 but got: $STATUS"
      return 1
    fi
  }
  run_test_case "menu exits with 130 on ESC" menu_esc_exits_with_130
fi

# Skip Exit test if /dev/tty is not functional
if [ "${WIZARDRY_TEST_IN_POCKET-0}" = "1" ]; then
  test_skip "menu exits with 130 on Exit item" "requires functional /dev/tty"
elif ! stty -g </dev/tty >/dev/null 2>&1; then
  test_skip "menu exits with 130 on Exit item" "requires functional /dev/tty"
else
  # Test that Exit menu item exits with code 130
  menu_exit_item_exits_with_130() {
    tmpdir=$(make_tempdir)
    
    stub_dir="$tmpdir/stubs"
    mkdir -p "$stub_dir"
    
    # Create custom await-keypress that presses enter (selects first item)
    cat > "$stub_dir/await-keypress" <<'STUB_EOF'
#!/bin/sh
printf 'enter\n'
STUB_EOF
    ...
    # Run menu with Exit item that uses "exit 130"
    PATH="$stub_dir:$ROOT_DIR/spells/cantrips:$WIZARDRY_IMPS_PATH:/bin:/usr/bin" run_cmd \
      "$ROOT_DIR/spells/cantrips/menu" "Test:" "Exit%exit 130"
    
    # Menu should exit with 130 when Exit item is selected
    if [ "$STATUS" -eq 130 ]; then
      return 0
    else
      TEST_FAILURE_REASON="expected exit code 130 but got: $STATUS"
  ...
  run_test_case "menu exits with 130 on Exit item" menu_exit_item_exits_with_130
fi
