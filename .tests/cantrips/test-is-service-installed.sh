...
test_missing_service_name_fails() {
  ...
  ASK_TEXT_RESPONSE="" IS_SERVICE_INSTALLED_ASK_TEXT="$stub_dir/ask-text" PATH="$WIZARDRY_CANTRIPS_PATH:$WIZARDRY_IMPS_PATH:$stub_dir:/bin:/usr/bin" _run_spell "spells/cantrips/is-service-installed"
...
test_reports_installed_service() {
  ...
  SERVICE_DIR="$service_dir" IS_SERVICE_INSTALLED_ASK_TEXT="$stub_dir/ask-text" PATH="$WIZARDRY_CANTRIPS_PATH:$WIZARDRY_IMPS_PATH:$stub_dir:/bin:/usr/bin" _run_spell "spells/cantrips/is-service-installed" demo
...
test_reports_missing_service() {
  ...
  write_ask_text_stub "$stub_dir"
  service_dir=$(mktemp -d "$WIZARDRY_TMPDIR/services.XXXXXX") || return 1
  SERVICE_DIR="$service_dir" IS_SERVICE_INSTALLED_ASK_TEXT="$stub_dir/ask-text" PATH="$WIZARDRY_CANTRIPS_PATH:$WIZARDRY_IMPS_PATH:$stub_dir:/bin:/usr/bin" _run_spell "spells/cantrips/is-service-installed" demo.service
