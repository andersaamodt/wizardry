...
STUB
  ...
  PATH="$ROOT_DIR/spells/.imps:$ROOT_DIR/spells/.imps/fs:$stubdir" CLIPBOARD_FILE="$clipboard" run_spell "spells/arcane/copy" "$file"
  assert_success || return 1
...
copy_fails_without_clipboard_tools() {
  ...
  PATH="$ROOT_DIR/spells/.imps:$ROOT_DIR/spells/.imps/fs:$stubdir" run_spell "spells/arcane/copy" "$file"
  assert_failure || return 1
