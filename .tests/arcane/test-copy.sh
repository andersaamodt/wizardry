...
copy_shows_usage() {
  ...
  _assert_output_contains "Usage: copy" || return 1
}

copy_requires_existing_file() {
  _run_spell "spells/arcane/copy" "$WIZARDRY_TMPDIR/nowhere.txt"
...
copy_rejects_directories_and_missing_target() {
  ...
  _run_spell "spells/arcane/copy" "$tmpdir/nonexistent-file"
  _assert_failure || return 1
  _assert_output_contains "That file does not exist." || return 1
}
...
STUB
  ...
  PATH="$stubdir:$PATH" CLIPBOARD_FILE="$clipboard" _run_spell "spells/arcane/copy" "$file"
  _assert_success || return 1
  # Normalize path for macOS compatibility (TMPDIR ends with /)
  normalized_file=$(printf '%s' "$file" | sed 's|//|/|g')
  _assert_output_contains "Copied $normalized_file to your clipboard." || return 1
...
STUB
  ...
  PATH="$ROOT_DIR/spells/cantrips:$ROOT_DIR/spells/.imps/cond:$ROOT_DIR/spells/.imps/out:$ROOT_DIR/spells/.imps/paths:$ROOT_DIR/spells/.imps/fs:$ROOT_DIR/spells/.imps/sys:$stubdir" CLIPBOARD_FILE="$clipboard" _run_spell "spells/arcane/copy" "$file"
  _assert_success || return 1
...
STUB
  ...
  _assert_output_contains "Copied $file_normalized to your clipboard." || return 1
...
copy_fails_without_clipboard_tools() {
  ...
  PATH="$ROOT_DIR/spells/cantrips:$ROOT_DIR/spells/.imps/cond:$ROOT_DIR/spells/.imps/out:$ROOT_DIR/spells/.imps/paths:$ROOT_DIR/spells/.imps/fs:$ROOT_DIR/spells/.imps/sys:$stubdir" _run_spell "spells/arcane/copy" "$file"
  _assert_failure || return 1
  _assert_error_contains "Your spell fizzles." || return 1
}
...
_run_test_case "copy shows usage text" copy_shows_usage
_run_test_case "copy requires an existing file" copy_requires_existing_file
_run_test_case "copy rejects directories and missing target" copy_rejects_directories_and_missing_target
_run_test_case "copy prefers pbcopy when available" copy_prefers_pbcopy
_run_test_case "copy falls back to xsel" copy_uses_xsel_when_available
_run_test_case "copy falls back to xclip when others are missing" copy_uses_xclip_when_others_missing
_run_test_case "copy prefers xsel over xclip" copy_prefers_xsel_over_xclip
_run_test_case "copy fails when no clipboard utility is present" copy_fails_without_clipboard_tools
_run_test_case "copy reports success message with original path" copy_reports_success_with_path


# Test via source-then-invoke pattern  
copy_help_via_sourcing() {
  _run_sourced_spell copy --help
  _assert_success || return 1
...
_run_test_case "copy works via source-then-invoke" copy_help_via_sourcing
_finish_tests
