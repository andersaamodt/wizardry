...
SH
  ...
  # Create get-attribute-batch stub that returns attributes in batch format
  cat >"$tmp/get-attribute-batch" <<'SH'
#!/bin/sh
file=$1
shift
case "$file" in
...
SH
  chmod +x "$tmp/get-attribute-batch"
...
SH
  ...
  # Create get-attribute-batch stub
  cat >"$tmp/get-attribute-batch" <<'SH'
#!/bin/sh
file=$1
shift
