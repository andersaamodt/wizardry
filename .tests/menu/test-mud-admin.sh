...
SH
  ...
  run_cmd env WIZARDRY_SUBMENU=1 REQUIRE_COMMAND="$tmp/require-command" PATH="$tmp:$PATH" MENU_LOG="$tmp/log" MENU_ESCAPE_STATUS=113 "$ROOT_DIR/spells/menu/mud-admin"
  assert_success
  args=$(cat "$tmp/log")
  case "$args" in
