...
SH
  chmod +x "$tmp/require"
  _run_cmd env REQUIRE_COMMAND="$tmp/require-command" PATH="$WIZARDRY_IMPS_PATH:$ROOT_DIR/spells/cantrips:$tmp:/bin:/usr/bin" MENU_LOG="$tmp/log" "$ROOT_DIR/spells/menu/mud-admin-menu"
  _assert_failure
...
mud_admin_menu_help_via_sourcing() {
  _run_sourced_spell mud-admin-menu --help
  _assert_success || return 1
...
_run_test_case "mud-admin-menu works via source-then-invoke" mud_admin_menu_help_via_sourcing
_finish_tests
