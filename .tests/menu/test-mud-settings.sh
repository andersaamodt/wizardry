...
SH
  ...
  args=$(cat "$tmp/log")
  case "$args" in
    *"MUD Settings:"*"player key"*"Change Player%choose-player"*"New Player%new-player"*'Exit%exit 130' ) : ;;
...
SH
  chmod +x "$tmp/require"
  run_cmd env REQUIRE_COMMAND="$tmp/require-command" PATH="$WIZARDRY_IMPS_PATH:$ROOT_DIR/spells/cantrips:$tmp:/bin:/usr/bin" MENU_LOG="$tmp/log" MUD_PLAYER=hero "$ROOT_DIR/spells/menu/mud-settings"
  assert_failure
