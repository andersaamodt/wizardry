...
SH
  ...
  run_cmd env REQUIRE_COMMAND="$tmp/require-command" PATH="$tmp:$PATH" MENU_LOG="$tmp/log" MUD_PLAYER="$player" MENU_ESCAPE_STATUS=113 "$ROOT_DIR/spells/menu/mud-settings"
  assert_success
  args=$(cat "$tmp/log")
  case "$args" in
