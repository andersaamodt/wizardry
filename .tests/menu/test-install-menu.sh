...
SH
  ...
  _run_cmd env PATH="$tmp:$PATH" INSTALL_MENU_ROOT="$install_root" INSTALL_MENU_DIRS="test" MENU_OUTPUT="$MENU_OUTPUT" "$ROOT_DIR/spells/menu/install-menu"
  _assert_success || return 1
...
install_menu_help_via_sourcing() {
  _run_sourced_spell install-menu --help
  _assert_success || return 1
...
_run_test_case "install-menu works via source-then-invoke" install_menu_help_via_sourcing
_finish_tests
