...
test_missing_read_magic() {
  ...
  stub=$(make_stub_dir)
  PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" _run_spell "spells/mud/look" "$WIZARDRY_TMPDIR"
...
test_missing_attributes_shows_defaults() {
  ...
  LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" _run_spell "spells/mud/look" "$test_room"
  _assert_success
...
test_output_ends_with_newline() {
  ...
  stub_ask_yn "$stub" 0
  stub_read_magic_missing "$stub"
  NO_COLOR=1 LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" _run_spell "spells/mud/look" "$test_room"
  _assert_success
...
test_home_description_defaults() {
  ...
  LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" \
...
test_other_home_description() {
  ...
  LOOK_READ_MAGIC="$stub/read-magic" HOME="$base_home" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" \
...
test_root_description() {
  ...
  stub_ask_yn "$stub" 1
  stub_read_magic_missing "$stub"
  LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" _run_spell "spells/mud/look" /
  _assert_success || return 1
...
EOF
  ...
  LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" _run_spell "spells/mud/look" "$WIZARDRY_TMPDIR"
...
look_help_via_sourcing() {
  _run_sourced_spell look --help
  _assert_success || return 1
...
_run_test_case "look works via source-then-invoke" look_help_via_sourcing
_finish_tests
