...
test_missing_read_magic() {
  ...
  stub=$(make_stub_dir)
  PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" run_spell "spells/mud/look" "$WIZARDRY_TMPDIR"
...
test_missing_attributes_shows_defaults() {
  ...
  LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" run_spell "spells/mud/look" "$test_room"
  assert_success
...
test_output_ends_with_newline() {
  ...
  stub_ask_yn "$stub" 0
  stub_read_magic_missing "$stub"
  NO_COLOR=1 LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" run_spell "spells/mud/look" "$test_room"
  assert_success
...
test_root_description() {
  ...
  stub_ask_yn "$stub" 1
  stub_read_magic_missing "$stub"
  LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" run_spell "spells/mud/look" /
  assert_success || return 1
...
EOF
  ...
  LOOK_READ_MAGIC="$stub/read-magic" PATH="$WIZARDRY_IMPS_PATH:$stub:$(wizardry_base_path):/bin:/usr/bin" run_spell "spells/mud/look" "$WIZARDRY_TMPDIR"
