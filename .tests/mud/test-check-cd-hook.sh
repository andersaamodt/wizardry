...
test_help() {
  run_spell "spells/mud/check-cd-hook" --help
  assert_success && assert_output_contains "Usage: check-cd-hook"
}
...
# <<< wizardry cd cantrip <<<
EOF
  WIZARDRY_RC_FILE="$rc_file" run_spell "spells/mud/check-cd-hook"
  assert_success
}
...
run_test_case "check-cd-hook prints usage" test_help
run_test_case "check-cd-hook returns success when hook is present" test_returns_success_when_hook_present
run_test_case "check-cd-hook returns failure when hook is absent" test_returns_failure_when_hook_absent
run_test_case "check-cd-hook returns failure when rc file is missing" test_returns_failure_when_rc_file_missing
run_test_case "check-cd-hook respects WIZARDRY_RC_FILE environment variable" test_respects_wizardry_rc_file_env
finish_tests
