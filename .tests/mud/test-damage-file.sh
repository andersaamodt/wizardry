...
test_help() {
  run_spell "spells/mud/damage-file" --help
  assert_success && assert_output_contains "Usage: damage-file"
}
...
test_damage_new_file() {
  ...
  run_spell "spells/mud/damage-file" "$test_file" 5
...
test_damage_accumulation() {
  ...
  run_spell "spells/mud/damage-file" "$test_file" 3
  assert_success
  
  # Apply damage second time - should accumulate
  run_spell "spells/mud/damage-file" "$test_file" 4
  assert_success && assert_output_contains "total: 7"
}

test_damage_invalid_args() {
  run_spell "spells/mud/damage-file"
  assert_failure
  
  run_spell "spells/mud/damage-file" "onlyonarg"
  assert_failure
}

test_damage_nonexistent_file() {
  run_spell "spells/mud/damage-file" "/nonexistent/file.txt" 5
...
test_damage_invalid_number() {
  ...
  run_spell "spells/mud/damage-file" "$test_file" "notanumber"
  assert_failure && assert_error_contains "must be a positive number"
}

run_test_case "damage-file prints usage" test_help
