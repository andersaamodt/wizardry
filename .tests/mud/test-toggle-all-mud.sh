...
test_help_shows_usage() {
  run_spell "spells/mud/toggle-all-mud" --help
...
test_enable_flag_enables_all() {
  tmp=$(make_tempdir)
  run_cmd env SPELLBOOK_DIR="$tmp" "$ROOT_DIR/spells/mud/toggle-all-mud" --enable
  assert_success || return 1
...
test_disable_flag_disables_all() {
  ...
  # Then disable all
  run_cmd env SPELLBOOK_DIR="$tmp" "$ROOT_DIR/spells/mud/toggle-all-mud" --disable
  assert_success || return 1
...
test_auto_toggle_enables_when_any_disabled() {
  ...
  run_cmd env SPELLBOOK_DIR="$tmp" "$ROOT_DIR/spells/mud/toggle-all-mud"
  assert_success || return 1
  assert_output_contains "All MUD features enabled" || return 1
}
